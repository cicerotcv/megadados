...
async def add_note(
    subject_id: int = Path(..., example=3),
    body: AddNote = Body(..., example={"note": 5}),
...
async def delete_note(
    ...
    if not old_note:
        raise HTTPException(
            404,
            detail=f"Note with id '{note_id}' does not exist in the subject with id '{subject_id}'",
        )
...
async def update_note(
    subject_id: int = Path(..., example=3),
    note_id: int = Path(..., example=3),
    data: AddNote = Body(..., example={"note": 5.6}),
    db: Session = Depends(get_db),
):
    subject = crud.find_subject_by_id(db, subject_id)
